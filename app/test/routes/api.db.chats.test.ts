...
function buildLoaderArgs(url: string) {
  ...
  return { request, params: {}, context: {} as any, url: new URL(request.url), pattern: '' };
}
...
describe('POST /api/db/chats', () => {
  ...
  it('returns 400 for invalid JSON body', async () => {
    ...
    const response = await action({ request, params: {}, context: {} as any, url: new URL(request.url), pattern: '' });
    expect(response.status).toBe(400);
...
describe('BOLT_DB_PATH env var (via resolveDbUrl)', () => {
  /*
   * resolveDbUrl is a pure function that reads process.env.BOLT_DB_PATH
  ...
  const originalEnv = process.env.BOLT_DB_PATH;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.BOLT_DB_PATH;
    } else {
      process.env.BOLT_DB_PATH = originalEnv;
    }
  });

  it('defaults to ./data/bolt.db when BOLT_DB_PATH is not set', () => {
    delete process.env.BOLT_DB_PATH;
    expect(resolveDbUrl()).toBe('file:./data/bolt.db');
  });

  it('uses BOLT_DB_PATH when set', () => {
    process.env.BOLT_DB_PATH = '/tmp/custom.db';
